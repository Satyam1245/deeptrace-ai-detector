...
def get_train_transforms(
    ...
    if use_heavy_augmentation:
        return Compose([
            ...
            RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0, p=0.3),
...
def get_test_time_augmentation_transforms(image_size: int = 224) -> list:
    ...
    return [
        ...
        Compose([
            RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0, p=1.0),
            Resize(image_size, image_size, always_apply=True),
            Normalize(
                mean=[0.485, 0.456, 0.406],
                std=[0.229, 0.224, 0.225],
        ...
        Compose([
            RandomBrightnessContrast(brightness_limit=0, contrast_limit=0.1, p=1.0),
            Resize(image_size, image_size, always_apply=True),
            Normalize(
                mean=[0.485, 0.456, 0.406],
                std=[0.229, 0.224, 0.225],
